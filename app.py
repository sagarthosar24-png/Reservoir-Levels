...
# --- 1. DATA TABLES ---
# (Using the arrays provided in previous steps)
...
# This keeps the levels consistent as you keep adding "next hours"
if 'curr_u_mcm' not in st.session_state:
    st.session_state.curr_u_mcm = 7.964 # Default start volume
if 'curr_l_mcm' not in st.session_state:
    st.session_state.curr_l_mcm = 3.290 # Default start volume
if 'history_log' not in st.session_state:
    st.session_state.history_log = []

# --- 3. LAYOUT ---
st.set_page_config(page_title="Sequential Operations Tool", layout="wide")
st.title("🎚️ Multi-Scenario Shift Calculator")

# Sidebar for Resetting
with st.sidebar:
    st.header("⚙️ Reset System")
    if st.button("Reset to Initial Levels"):
        st.session_state.curr_u_mcm = u_content_data[(np.abs(u_level_data - 94.450)).argmin()]
        st.session_state.curr_l_mcm = l_content_data[(np.abs(l_level_data - 90.000)).argmin()]
        st.session_state.history_log = []
        st.rerun()

# --- 4. INPUT SECTION (The "Next Step" Config) ---
st.subheader("Compute Next Operation Block")
c1, c2, c3, c4 = st.columns(4)

with c1:
    duration = st.number_input("Duration (Hrs)", min_value=1.0, value=1.0, step=1.0)
with c2:
    block_gen = st.number_input("Generation for this block (MUS)", value=0.179, format="%.3f")
with c3:
    gate_open = st.selectbox("Gate Status", ["Open", "Closed"], index=0)
with c4:
    st.write("##") # Spacer
    calc_button = st.button("Calculate Block", type="primary", use_container_width=True)

# --- 5. CALCULATION ENGINE ---
if calc_button:
    u_mcm = st.session_state.curr_u_mcm
    l_mcm = st.session_state.curr_l_mcm
    
    # 1. Block Parameters
    u_conv = 0.820
    # Inflow rate for this specific block duration
    inflow_rate_hr = (block_gen * u_conv) / duration
    
    # Simulation settings (Fixed 10-min resolution)
    steps = int(duration * 6) 
    dt = 1/6 # hour per step
    
    total_gate_vol = 0.0
    
    for _ in range(steps):
        # A. Get current RLs
        idx_u = (np.abs(u_content_data - u_mcm)).argmin()
        idx_l = (np.abs(l_content_data - l_mcm)).argmin()
        rl_u = u_level_data[idx_u]
        rl_l = l_level_data[idx_l]
        
        # B. Dynamic Head Logic
        diff = rl_u - rl_l
        if gate_open == "Closed": rate = 0.0
        elif diff > 3.0: rate = 0.17
        elif 2.0 <= diff <= 3.0: rate = 0.15
        elif 1.5 <= diff < 2.0: rate = 0.12
        else: rate = 0.08
        
        # C. Apply changes
        u_mcm += (inflow_rate_hr * dt)
        u_mcm -= (rate * dt)
        l_mcm += (rate * dt)
        total_gate_vol += (rate * dt)

    # 2. Update Session State
    st.session_state.curr_u_mcm = u_mcm
    st.session_state.curr_l_mcm = l_mcm
    
    # Final RLs for the log
    final_u_rl = u_level_data[(np.abs(u_content_data - u_mcm)).argmin()]
    final_l_rl = l_level_data[(np.abs(l_content_data - l_mcm)).argmin()]
    
    # 3. Save to History
    st.session_state.history_log.append({
        "Block Duration": f"{duration} hr",
        "Gen (MUS)": block_gen,
        "Gate": gate_open,
        "End Upper RL": f"{final_u_rl:.3f}",
        "End Lower RL": f"{final_l_rl:.3f}",
        "Gate Transfer": f"{total_gate_vol:.3f} MCM"
    })
...
final_u = u_level_data[(np.abs(u_content_data - st.session_state.curr_u_mcm)).argmin()]
final_l = l_level_data[(np.abs(l_content_data - st.session_state.curr_l_mcm)).argmin()]

m1.metric("Current Upper Lake RL", f"{final_u:.3f} m")
m2.metric("Current Lower Res. RL", f"{final_l:.3f} m")

if st.session_state.history_log:
    st.subheader("📜 Shift Operation Log")
    df = pd.DataFrame(st.session_state.history_log)
    st.table(df)
else:
    st.info("No blocks calculated yet. Enter parameters above and click 'Calculate Block'.")
